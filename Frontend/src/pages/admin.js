...
export async function loadAdminPage(session, { onLogout, sub = 'accounts', editId = null }) {
  ...
  if (sub === 'ml') return loadMlMetricsPage(session, { onLogout });
...
async function loadMlMetricsPage(session, { onLogout }) {
    ...
    mainHtml: `
      <div class="page-header">
        <div>
          ...
          <p class="page-lead">Train/test metrics for Random Forest vs Logistic Regression (proposal objectives 2–3).</p>
        </div>
        <div class="page-actions">
          <button type="button" id="btn-run-eda" class="btn btn-secondary">Run EDA</button>
          <button type="button" id="btn-train-model" class="btn btn-primary">Retrain models</button>
        </div>
      </div>
      <section class="panel" id="ml-meta-host">${loadingBlock('Loading model metrics…')}</section>
      <div id="admin-modal-host"></div>
    `,
  });
  bindAdminShell(session, onLogout);

  const host = document.getElementById('ml-meta-host');

  async function refresh() {
    host.innerHTML = loadingBlock('Loading model metrics…');
    try {
      const meta = await api.getModelMeta();
      const rf = meta.rf_metrics || {};
      const lr = meta.lr_metrics || {};
      const cm = rf.confusion_matrix || {};
      const sm = meta.statsmodels_logit || {};
      const protocol = meta.train_test_protocol || {};
      const prep = meta.preprocessing || {};
      const importance = Object.entries(meta.feature_importance || {})
        .sort((a, b) => b[1] - a[1])
        .slice(0, 8);
      ...
      host.innerHTML = `
        <div class="metric-grid">
          <div class="metric-tile">
            ...
            <strong>${escapeHtml(meta.primary_model || 'random_forest')}</strong>
            <span class="metric-sub">v${escapeHtml(String(meta.version || '—'))}</span>
          </div>
          <div class="metric-tile">
            <span class="metric-label">RF ROC-AUC</span>
            <strong>${fmtMetric(rf.roc_auc)}</strong>
            <span class="metric-sub">Acc ${fmtMetric(rf.accuracy)} · F1 ${fmtMetric(rf.f1)}</span>
          </div>
          <div class="metric-tile">
            <span class="metric-label">LR ROC-AUC</span>
            <strong>${fmtMetric(lr.roc_auc)}</strong>
            <span class="metric-sub">Acc ${fmtMetric(lr.accuracy)} · F1 ${fmtMetric(lr.f1)}</span>
          </div>
          <div class="metric-tile">
            <span class="metric-label">RF vs LR</span>
            <strong>${meta.rf_outperforms_baseline ? 'RF wins' : 'Baseline competitive'}</strong>
            <span class="metric-sub">${escapeHtml(meta.training_source || '')}</span>
          </div>
        </div>

        <h3 class="section-title">Hold-out comparison</h3>
        <div class="table-wrap">
          <table class="data-table">
            ...
            <tbody>
              <tr>
                <td>Random Forest (primary)</td>
                <td>${fmtMetric(rf.accuracy)}</td>
                <td>${fmtMetric(rf.precision_score)}</td>
                <td>${fmtMetric(rf.recall)}</td>
                <td>${fmtMetric(rf.f1)}</td>
                <td>${fmtMetric(rf.roc_auc)}</td>
              </tr>
              <tr>
                <td>Logistic Regression (baseline)</td>
                <td>${fmtMetric(lr.accuracy)}</td>
                <td>${fmtMetric(lr.precision_score)}</td>
                <td>${fmtMetric(lr.recall)}</td>
                <td>${fmtMetric(lr.f1)}</td>
                <td>${fmtMetric(lr.roc_auc)}</td>
              </tr>
            </tbody>
          </table>
        </div>

        <h3 class="section-title">Confusion matrix (Random Forest)</h3>
        <p class="page-lead">TN ${cm.tn ?? '—'} · FP ${cm.fp ?? '—'} · FN ${cm.fn ?? '—'} · TP ${cm.tp ?? '—'}</p>
        <div class="table-wrap">
          <table class="data-table">
            <thead><tr><th></th><th>Pred higher risk</th><th>Pred creditworthy</th></tr></thead>
            <tbody>
              <tr><th>Actual higher risk</th><td>${cm.tn ?? '—'}</td><td>${cm.fp ?? '—'}</td></tr>
              <tr><th>Actual creditworthy</th><td>${cm.fn ?? '—'}</td><td>${cm.tp ?? '—'}</td></tr>
            </tbody>
          </table>
        </div>

        <h3 class="section-title">Train / test protocol</h3>
        <ul class="plain-list">
          <li>Split: ${(protocol.test_size ?? 0.2) * 100}% hold-out · train ${protocol.train_rows ?? '—'} / test ${protocol.test_rows ?? '—'}</li>
          <li>CV folds: ${protocol.cv_folds ?? '—'} · selection metric: ${escapeHtml(protocol.selection_metric || 'roc_auc')}</li>
          <li>Preprocessing: ${escapeHtml(prep.missing_values || '—')}; ${escapeHtml(prep.outliers || '')}</li>
          <li>Scaling: ${escapeHtml(prep.scaling || '—')}</li>
          <li>PII: ${escapeHtml(prep.pii_policy || 'Feature matrix excludes PII')}</li>
          ${sm.available ? `<li>Statsmodels Logit ROC-AUC: ${fmtMetric(sm.roc_auc)} (classical benchmark)</li>` : ''}
        </ul>

        ${importance.length ? `
          <h3 class="section-title">Top feature importance (RF)</h3>
          <ul class="plain-list">
            ${importance.map(([k, v]) => `<li>${escapeHtml(k)} — ${pct(v)}</li>`).join('')}
          </ul>
        ` : ''}
      `;
    } catch (err) {
      host.innerHTML = errorBlock('Could not load ML metrics', getErrorMessage(err), 'retry-ml');
      document.getElementById('retry-ml')?.addEventListener('click', refresh);
    }
  }
  ...
  document.getElementById('btn-train-model')?.addEventListener('click', async () => {
    ...
    try {
      ...
      showToast(result.message || 'Training complete.', 'success');
      await refresh();
    } catch (err) {
      showToast(getErrorMessage(err, 'Training failed'), 'error');
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = 'Retrain models'; }
    }
  });

  document.getElementById('btn-run-eda')?.addEventListener('click', async () => {
    const btn = document.getElementById('btn-run-eda');
    if (btn) { btn.disabled = true; btn.textContent = 'Running EDA…'; }
    try {
      const summary = await api.runEda();
      showToast(`EDA done (${summary.rows || 0} rows). Figures saved on server.`, 'success');
    } catch (err) {
      showToast(getErrorMessage(err, 'EDA failed'), 'error');
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = 'Run EDA'; }
    }
  });
